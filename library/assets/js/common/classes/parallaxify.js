		...
		_repositionElements: function() {
			...
			for (i = this.elements.length - 1; i >= 0; i--) {
				...
				if (this.options.horizontalParallax) {
					newPositionLeft = (moveHorizontal * element.parallaxDistanceX / 2) + element.originalPositionLeft;
				} else {
					newPositionLeft = element.originalPositionLeft;
				}

				if (this.options.verticalParallax) {
					newPositionTop = (moveVertical * element.parallaxDistanceY / 2) + element.originalPositionTop;
				} else {
			...
			for (i = this.backgrounds.length - 1; i >= 0; i--) {
				...
				bgLeft = (this.options.horizontalParallax ?(moveHorizontal * background.parallaxDistanceX / 2)  + background.originalBackgroundPositionLeft + 'px' : background.originalValueLeft);
				bgTop = (this.options.verticalParallax ? (moveVertical * background.parallaxDistanceY / 2)  + background.originalBackgroundPositionTop + 'px' : background.originalValueTop);
